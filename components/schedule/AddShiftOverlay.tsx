...
import { setTB } from "../../lib/services/timeblocks";
...
export function AddShiftOverlay({ isOpen, onClose }: Props) {
    ...
    const handleSave = async () => {
        if (
            ...
            !endTime ||
            !shiftType ||
            (openToVolunteers && !maxVolunteers)
        ) {
        ...
        await createTB(
            startTime,
            endTime,
            openToVolunteers ? Number(maxVolunteers) : 0,
        );
        onClose();
    };
                ...
                <div className="p-4">
                    ...
                    <label className="flex items-center gap-2 mb-3 cursor-pointer">
                        <input
                            ...
                            onChange={(e) =>
                                setOpenToVolunteers(e.target.checked)
                            }
                            className="accent-[#02AFC7]"
                        />
                        <span className="text-sm text-slate-700">
